...
logging.getLogger("httpx").setLevel(logging.WARNING) logging.getLogger("aiohttp.access").setLevel(logging.WARNING)
...
TOKEN = settings["TOKEN"] ADMIN_ID = settings["ADMIN_ID"] BASE_URL = settings.get("BASE_URL", "http://127.0.0.1") HTTP_PORT = settings.get("HTTP_PORT", 8080)

STREAMS_FILE = "streams_pro.json" HLS_DIR = "/tmp/hls"
...
streams = {} viewer_last_seen = defaultdict(dict) monitor_tasks = {} last_panel_text = {}
...
if os.path.exists(STREAMS_FILE): try: with open(STREAMS_FILE, "r", encoding="utf-8") as f: data = json.load(f)
...
def get_system_status(): cpu = psutil.cpu_percent(interval=0.2)
...
async def track_viewer(request, stream_name): forwarded = request.headers.get("X-Forwarded-For") ip = forwarded.split(",")[0].strip() if forwarded else request.remote
...
def clean_viewers(): now = time.time()
...
async def handle_hls(request): name = request.match_info["name"] file = request.match_info.get("file", "index.m3u8")
...
MAIN_KEYBOARD = ReplyKeyboardMarkup( [ [KeyboardButton("📺 HLS"), KeyboardButton("📡 RTMP")], [KeyboardButton("➕ إضافة"), KeyboardButton("🖥 مراقبة")], ], resize_keyboard=True, )

def main_menu(): return InlineKeyboardMarkup([ [InlineKeyboardButton("📺 بثوث HLS", callback_data="list_hls")], [InlineKeyboardButton("📡 بثوث RTMP", callback_data="list_rtmp")], [InlineKeyboardButton("➕ إضافة بث", callback_data="add_stream")], [InlineKeyboardButton("🖥 مراقبة السيرفر", callback_data="monitor")], ])

def navigation_row(): return [ InlineKeyboardButton("HLS", callback_data="list_hls"), InlineKeyboardButton("RTMP", callback_data="list_rtmp"), InlineKeyboardButton("➕", callback_data="add_stream"), InlineKeyboardButton("🖥", callback_data="monitor"), ]

def stream_list(stream_type): kb = []
...
def stream_panel_keyboard(sid, s): mode = s.get("mode", "copy")
...
kb.append([
    InlineKeyboardButton(toggle_text, callback_data=f"togglemode_{sid}")
])

kb.append([
    InlineKeyboardButton("🗑 حذف", callback_data=f"delete_{sid}")
])
...
async def check_admin(update): user_id = update.effective_user.id
...
async def start(update, context): if not await check_admin(update): return
...
async def start_monitor_live(query, chat_id): if chat_id in monitor_tasks: monitor_tasks[chat_id].cancel()
...
async def stop_monitor(chat_id, query): if chat_id in monitor_tasks: monitor_tasks[chat_id].cancel() del monitor_tasks[chat_id]
...
async def update_panel_message(sid, bot): s = streams.get(sid)

if not s:
    return

if not s.get("panel_msg_id"):
    return
...
def build_hls_copy(src, ua, output): return [ "ffmpeg", "-loglevel", "warning", "-re", "-user_agent", ua, "-thread_queue_size", "4096", "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5", "-rw_timeout", "10000000", "-fflags", "+genpts+discardcorrupt", "-i", src, "-map", "0:v?", "-map", "0:a?", "-c:v", "copy", "-c:a", "copy", "-f", "hls", "-hls_time", "4", "-hls_list_size", "6", "-hls_delete_threshold", "2", "-hls_flags", "delete_segments+append_list", output ]

def build_hls_transcode(src, ua, logo, output): cmd = [ "ffmpeg", "-loglevel", "warning", "-re", "-user_agent", ua, "-thread_queue_size", "4096", "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5", "-rw_timeout", "10000000", "-fflags", "+genpts+discardcorrupt", "-i", src, ]
...
def build_rtmp_copy(src, ua, rtmp_url): return [ "ffmpeg", "-loglevel", "warning", "-re", "-user_agent", ua, "-thread_queue_size", "4096", "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5", "-rw_timeout", "10000000", "-fflags", "+genpts+discardcorrupt", "-i", src, "-c:v", "copy", "-c:a", "copy", "-f", "flv", rtmp_url ]

def build_rtmp_transcode(src, ua, logo, rtmp_url): cmd = [ "ffmpeg", "-loglevel", "warning", "-re", "-user_agent", ua, "-thread_queue_size", "4096", "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5", "-rw_timeout", "10000000", "-fflags", "+genpts+discardcorrupt", "-i", src, ]
...
async def start_stream(sid, bot): s = streams[sid]
...
async def stop_stream(sid, bot): s = streams.get(sid)
...
async def button_handler(update, context): q = update.callback_query await q.answer()
...
async def msg_handler(update, context): if not await check_admin(update): return
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): tb = ''.join(traceback.format_exception( None, context.error, context.error.traceback ))
...
async def startup(): await start_http_server()

if name == "main": loop = asyncio.new_event_loop() asyncio.set_event_loop(loop)
